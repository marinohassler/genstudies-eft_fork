...
variables = {
    ...
    'cosTheta': {
        'function': lambda t: abs(t.cosTheta),
        'axis': hist.axis.Regular(50, 0, 1, name='cosTheta')
    }
}
...
histos = {
    op: {
        'sm': [hist.Hist(mll_axis, storage=hist.storage.Weight()) for _ in cos_bins],
        'linear': [hist.Hist(mll_axis, storage=hist.storage.Weight()) for _ in cos_bins],
        'full': [hist.Hist(mll_axis, storage=hist.storage.Weight()) for _ in cos_bins],
    }
    for op in operators
}
