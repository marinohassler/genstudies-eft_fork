...
cos_bins = [(0.0, 0.5), (0.5, 1.0)]# (0.5, 0.75), (0.75, 1.0)]
...
for op in operators:
    ...
    for idx, (cmin, cmax) in enumerate(cos_bins):
        ...
        ax[0].stairs(h_full.values(), edges, label=f'{op} full |cosθ| in [{cmin}, {cmax})', linestyle='--', color=colors[idx])

        denominator = np.where(h_sm.values() > 0., h_sm.values(), 1e-6)
        ratio = h_full.values() / denominator
        all_ratios.append(ratio)
        ax[1].stairs(ratio, edges, edgecolor=colors[idx], fill=False, linewidth=1.0, label=f'Full / SM [{cmin},{cmax})')
    ...
    plt.tight_layout()
    fig.savefig(f"{args.output}/mll_in_costheta_ratio_{op}.png", bbox_inches="tight")
    plt.close(fig)
